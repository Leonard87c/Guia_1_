...
public class Parqueadero {
     ...
     * Hora del día
     */
    public static final int HORAS_POR_DIA = 23;
    ...
    public void avanzarHora() {
        ...
        horaActual++;
        if (horaActual > HORAS_POR_DIA){
            horaActual = 0;
        }
        if (horaActual < HORA_INICIAL || horaActual >= HORA_CIERRE){
            abierto = false;
        } else {
            abierto = true;
        }
    ...
    /**
     * Carro con más tiempo en el parqueadero
     */
    int iTiempoParquadero = 0;  //Mayor tiempo

    String sPlacaCarro = null; // placa con mas tiempo

    for (int i = 0; i < TAMANO; i++) {

        if (puestos[i].estaOcupado()){
            Carro vCarro = puestos [i].darCarro();

            int iTiempoTmp = vCarro.darTiempoEnParqueadero(horaActual);
            if (iTiempoTmp > iTiempoParquadero){
                iTiempoParquadero = iTiempoTmp;
                sPlacaCarro = vCarro.darPlaca();
            }
        }
    }
    if (sPlacaCarro == null) sPlacaCarro = "Lo sentimos, no hay carros en el parqueadero";

    else sPlacaCarro = "El carro con mas tiempo en el parqueadero es el de la placa: " + sPlacaCarro;
    ...
    public String metodo2() {
        ...
        String retorno = "Hay puestos disponibles";

        int iLibres = calcularPuestosLibres();
        if (iLibres == 0) retorno = "El parqueadero esta lleno";
